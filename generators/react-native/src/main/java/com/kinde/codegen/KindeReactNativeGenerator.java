...
public class KindeReactNativeGenerator extends AbstractTypeScriptClientCodegen {
	...
	protected String projectName = "@kinde-oss/react-native-sdk-0-6x";
	...
	public KindeReactNativeGenerator() {
		...
		this.cliOptions.add(
				new CliOption(CodegenConstants.SOURCE_FOLDER, CodegenConstants.SOURCE_FOLDER_DESC)
						.defaultValue("src"));
		this.cliOptions.add(new CliOption(CodegenConstants.INVOKER_PACKAGE,
		...
		this.cliOptions.add(new CliOption(PREFIX_PARAMETER_INTERFACES,
				...
				SchemaTypeUtil.BOOLEAN_TYPE).defaultValue(Boolean.FALSE.toString()));
		this.cliOptions.add(new CliOption(WITHOUT_RUNTIME_CHECKS,
				...
				SchemaTypeUtil.BOOLEAN_TYPE).defaultValue(Boolean.FALSE.toString()));
		this.cliOptions.add(new CliOption(SAGAS_AND_RECORDS,
				"Setting this property to true will generate additional files for use with redux-saga and immutablejs.",
				SchemaTypeUtil.BOOLEAN_TYPE).defaultValue(Boolean.FALSE.toString()));
		this.cliOptions
	...
	public String getName() {
		return "kinde-react-native";
	}

	@Override
	public String getHelp() {
		return "Generates a react-native client library.";
	}
	...
	public String getPassthroughSuffix() {
		return detectPassthroughModelsWithSuffixAndField != null
				? detectPassthroughModelsWithSuffixAndField.split("\\.")[0]
				: null;
	}

	public String getPassthroughField() {
		return detectPassthroughModelsWithSuffixAndField != null
				? detectPassthroughModelsWithSuffixAndField.split("\\.")[1]
				: null;
	}
	...
	public void setDetectPassthroughModelsWithSuffixAndField(
			String detectPassthroughModelsWithSuffixAndField) {
	...
	public boolean isUniqueIdAccordingToNameSuffix(String name) {
		...
		return "id".equals(name) || "ids".equals(name)
	...
	public void processOpts() {
		...
		this.apiPackage = sourceFolder + File.separator + "apis";
		this.modelPackage = sourceFolder + File.separator + "models";

		supportingFiles.add(
				new SupportingFile("index.mustache", sourceFolder + File.separator, "index.ts"));
		...
		writePropertyBack(CodegenConstants.USE_SINGLE_REQUEST_PARAMETER,
				getUseSingleRequestParameter());

		if (additionalProperties.containsKey(PREFIX_PARAMETER_INTERFACES)) {
			this.setPrefixParameterInterfaces(
					convertPropertyToBoolean(PREFIX_PARAMETER_INTERFACES));
		}
		...
		if (additionalProperties.containsKey(SAGAS_AND_RECORDS)) {
			...
			if (this.getSagasAndRecords()) {
				...
				supportingFiles.add(new SupportingFile("runtimeSagasAndRecords.mustache", sourceDir,
						"runtimeSagasAndRecords.ts"));
				supportingFiles.add(new SupportingFile("ApiEntitiesRecord.mustache", sourceDir,
						"ApiEntitiesRecord.ts"));
				supportingFiles.add(new SupportingFile("ApiEntitiesReducer.mustache", sourceDir,
						"ApiEntitiesReducer.ts"));
				supportingFiles.add(new SupportingFile("ApiEntitiesSelectors.mustache", sourceDir,
						"ApiEntitiesSelectors.ts"));

				if (additionalProperties
						.containsKey(DETECT_PASSTHROUGH_MODELS_WITH_SUFFIX_AND_FIELD)) {
					this.setDetectPassthroughModelsWithSuffixAndField((String) additionalProperties
							.get(DETECT_PASSTHROUGH_MODELS_WITH_SUFFIX_AND_FIELD));
				}
				if (additionalProperties.containsKey(INFER_UNIQUE_ID_FROM_NAME_SUFFIX)) {
					this.setInferUniqueIdFromNameSuffix(
							convertPropertyToBoolean(INFER_UNIQUE_ID_FROM_NAME_SUFFIX));
				}
				if (additionalProperties.containsKey(INFER_ENTITY_FROM_UNIQUE_ID_WITH_NAME)) {
					this.setInferEntityFromUniqueIdWithName((String) additionalProperties
							.get(INFER_ENTITY_FROM_UNIQUE_ID_WITH_NAME));
				}
				if (additionalProperties.containsKey(PACKAGE_AS_SOURCE_ONLY_LIBRARY)) {
					this.setPackageAsSourceOnlyLibrary(
							convertPropertyToBoolean(PACKAGE_AS_SOURCE_ONLY_LIBRARY));
				}
				...
				if (this.getPackageAsSourceOnlyLibrary()) {
					supportingFiles
							.add(new SupportingFile("sourceLibraryIndex.mustache", "", "index.ts"));
				}
			}
		}
		...
		addSupportingFilesForAndroid();
		addSupportingFilesForTests();
		...
		for (ExtendedCodegenModel rootModel : allModels) {
			...
			for (CodegenProperty cpVar : rootModel.vars) {
				...
				if (var.isModel && entityModelClassnames.indexOf(var.dataType) != -1) {
					var.isEntity = true;
				} else if (var.isArray && var.items.isModel
						&& entityModelClassnames.indexOf(var.items.dataType) != -1) {
	...
	private void autoSetDefaultValueForProperty(ExtendedCodegenProperty var) {
		if (var.isArray || var.isModel) {
			...
			if (var.allowableValues != null
					&& var.allowableValues.get("enumVars") instanceof ArrayList
					&& ((ArrayList) var.allowableValues.get("enumVars"))
							.get(0) instanceof HashMap) {
				var.defaultValue = var.dataTypeAlternate + "."
						+ ((HashMap<String, String>) ((ArrayList) var.allowableValues
								.get("enumVars")).get(0)).get("name");
			}
		}
	}
	...
	@Override
	public List<CodegenParameter> fromRequestBodyToFormParameters(RequestBody body,
			Set<String> imports) {
	...
	@Override
	public ExtendedCodegenOperation fromOperation(String path, String httpMethod,
		...
		if (this.getSagasAndRecords()) {
			...
			if (methodResponse != null) {
				...
				if (schema != null) {
					...
					} else if (this.getDetectPassthroughModelsWithSuffixAndField() != null
							&& op.returnBaseType.length() > this.getPassthroughSuffix().length()
							&& op.returnBaseType.endsWith(this.getPassthroughSuffix())) {
						boolean foundMatch = false;
						for (CodegenProperty var : cm.vars) {
				...
				if (!op.hasReturnPassthroughVoid) {
					Schema responseSchema = unaliasSchema(
							ModelUtils.getSchemaFromResponse(methodResponse), schemaMapping);
	...
	@Override
	public OperationsMap postProcessOperationsWithModels(OperationsMap operations,
			List<ModelMap> allModels) {
		// Add supporting file only if we plan to generate files in /apis
		if (!operations.isEmpty() && !addedApiIndex) {
			addedApiIndex = true;
			supportingFiles.add(new SupportingFile("apis.index.mustache",
					apiPackage().replace('.', File.separatorChar), "index.ts"));
			if (this.getSagasAndRecords()) {
				supportingFiles.add(new SupportingFile("sagaApiManager.mustache",
						apiPackage().replace('.', File.separatorChar), "SagaApiManager.ts"));
				supportingFiles.add(new SupportingFile("allSagas.mustache",
						apiPackage().replace('.', File.separatorChar), "allSagas.ts"));
			}
		}
		...
		if (!allModels.isEmpty() && !addedModelIndex) {
			...
			supportingFiles.add(new SupportingFile("models.index.mustache",
	...
	private ExtendedCodegenModel processCodeGenModel(ExtendedCodegenModel cm) {
		if (this.getSagasAndRecords()) {
			...
			for (CodegenProperty cpVar : cm.vars) {
				ExtendedCodegenProperty var = (ExtendedCodegenProperty) cpVar;
				if (propertiesToKeepAsJSObject != null
						&& Arrays.asList(propertiesToKeepAsJSObject).contains(var.name)) {
			...
			if (returnPassthrough instanceof String) {
				if (((String) returnPassthrough).isEmpty()) {
					...
					&& cm.name.length() > this.getPassthroughSuffix().length()
					&& cm.name.endsWith(this.getPassthroughSuffix())) {
				boolean foundMatch = false;
				for (CodegenProperty var : cm.vars) {
					if (var.name.equals(this.getPassthroughField())) {
						foundMatch = true;
	...
	private boolean processCodegenProperty(ExtendedCodegenProperty var, String parentClassName,
			Object xEntityId) {
		...
		if (this.getSagasAndRecords()) {
			...
			if (var.isArray) {
				...
				if (var.itemsAreNullable()) {
					var.dataTypeAlternate = var.dataTypeAlternate.replace(newItemsDataType,
							newItemsDataType + " | null");
				}
			} else if (var.isEnum) {
			...
			for (CodegenOperation _op : operations.getOperations().getOperation()) {
				...
				if (op.returnPassthrough != null && op.returnBaseTypeAlternate != null) {
					if (op.returnTypeSupportsEntities
							&& !existingRecordClassNames.contains(op.returnBaseTypeAlternate)) {
						additionalPassthroughImports.add(op.returnBaseTypeAlternate);
					} else if (!op.returnTypeSupportsEntities
							&& !existingClassNames.contains(op.returnBaseTypeAlternate)) {
						additionalPassthroughImports.add(op.returnBaseTypeAlternate);
					}
				}
			}
			...
			for (CodegenParameter cpParam : op.allParams) {
				...
				if (param.isArray) {
					...
					if (param.items.isModel) {
						...
						param.dataTypeAlternate = param.dataTypeAlternate
								.replace(param.items.dataType, newItemsDataType);
					} else if (param.items.isEnum) {
						newItemsDataType = param.datatypeWithEnum.substring(
								param.datatypeWithEnum.lastIndexOf("<") + 1,
								param.datatypeWithEnum.indexOf(">"));
					...
					if (param.itemsAreNullable()) {
						param.dataTypeAlternate = param.dataTypeAlternate.replace(newItemsDataType,
								newItemsDataType + " | null");
					}
	...
	class ExtendedCodegenParameter extends CodegenParameter {
		...
		public boolean itemsAreUniqueId() {
			return KindeReactNativeGenerator.itemsAreUniqueId(this.items);
		}

		public boolean itemsAreNullable() {
			return KindeReactNativeGenerator.itemsAreNullable(this.items);
		}

		public String getItemsDataType() {
			return KindeReactNativeGenerator.getItemsDataType(this.items);
		}
		...
		public boolean equals(Object o) {
			...
			return result && isUniqueId == that.isUniqueId
	...
	class ExtendedCodegenProperty extends CodegenProperty {
		...
		public boolean equals(Object o) {
			...
			return result && isEntity == that.isEntity && isUniqueId == that.isUniqueId
	...
	class ExtendedCodegenOperation extends CodegenOperation {
		boolean hasReturnPassthroughVoid, returnTypeSupportsEntities, returnTypeIsModel,
				returnTypeIsArray;
		...
		public boolean equals(Object o) {
			...
			return result && hasReturnPassthroughVoid == that.hasReturnPassthroughVoid
		...
		public int hashCode() {
			int superHash = super.hashCode();
			return Objects.hash(superHash, returnPassthrough, hasReturnPassthroughVoid,
					returnTypeSupportsEntities, returnTypeIsArray, returnTypeIsModel,
					returnTypeAlternate, returnBaseTypeAlternate);
		}
	...
	class ExtendedCodegenModel extends CodegenModel {
		...
		public boolean equals(Object o) {
			...
			return result && isEntity == that.isEntity
		...
		public int hashCode() {
			int superHash = super.hashCode();
			return Objects.hash(superHash, isEntity, returnPassthrough, hasReturnPassthroughVoid,
					getModelImports());
		}
	...
	private void addSupportingFilesForTests() {
		supportingFiles.add(new SupportingFile("index.spec.mustache", createPath("__tests__"),
				"index.spec.ts"));
	}
	...
	private void addSupportingFilesForSDK() {
		...
		supportingFiles.add(new SupportingFile("SDK/Storage/RNStorage.mustache",
				createPath(sourceFolder, sdkFolder, "Storage"), "RNStorage.ts"));
		supportingFiles.add(new SupportingFile("SDK/Storage/ExpoStorage.mustache",
				createPath(sourceFolder, sdkFolder, "Storage"), "ExpoStorage.ts"));
	...
	private void addSupportingFilesForAndroid() {
		supportingFiles.add(new SupportingFile("android/build.gradle", createPath(androidFolder),
				"build.gradle"));
		supportingFiles.add(new SupportingFile("android/gradle.properties",
				createPath(androidFolder), "gradle.properties"));
		supportingFiles
				.add(new SupportingFile("android/gradlew", createPath(androidFolder), "gradlew"));
		supportingFiles.add(new SupportingFile("android/gradlew.bat", createPath(androidFolder),
				"gradlew.bat"));
		supportingFiles.add(new SupportingFile("android/settings.gradle", createPath(androidFolder),
				"settings.gradle"));

		// app folder
		supportingFiles.add(new SupportingFile(
				"android/app/src/main/java/com/kindesdkrn/MainActivity.java",
				createPath(androidFolder), "app/src/main/java/com/kindesdkrn/MainActivity.java"));
		supportingFiles.add(
				new SupportingFile("android/app/src/main/java/com/kindesdkrn/MainApplication.java",
						createPath(androidFolder),
						"app/src/main/java/com/kindesdkrn/MainApplication.java"));
		supportingFiles.add(new SupportingFile("android/app/src/main/AndroidManifest.xml",
				createPath(androidFolder), "app/src/main/AndroidManifest.xml"));
		supportingFiles.add(new SupportingFile("android/app/build.gradle",
				createPath(androidFolder), "app/build.gradle"));
		supportingFiles
				.add(new SupportingFile("android/app/BUCK", createPath(androidFolder), "app/BUCK"));
		supportingFiles.add(new SupportingFile("android/app/proguard-rules.pro",
				createPath(androidFolder), "app/proguard-rules.pro"));

		// gradle folder
		supportingFiles.add(new SupportingFile("android/gradle/wrapper/gradle-wrapper.jar",
				createPath(androidFolder), "gradle/wrapper/gradle-wrapper.jar"));
		supportingFiles.add(new SupportingFile("android/gradle/wrapper/gradle-wrapper.properties",
				createPath(androidFolder), "gradle/wrapper/gradle-wrapper.properties"));
	}

	private void addSupportingFilesForIOS() {
		// KindeSDKRN folder
		supportingFiles.add(new SupportingFile("ios/KindeSDKRN/Base.lproj/LaunchScreen.xib",
				createPath(iOSFolder), "KindeSDKRN/Base.lproj/LaunchScreen.xib"));
		supportingFiles.add(new SupportingFile(
				"ios/KindeSDKRN/Images.xcassets/AppIcon.appiconset/Contents.json",
				createPath(iOSFolder),
				"KindeSDKRN/Images.xcassets/AppIcon.appiconset/Contents.json"));
		supportingFiles.add(new SupportingFile("ios/KindeSDKRN/Images.xcassets/Contents.json",
				createPath(iOSFolder), "KindeSDKRN/Images.xcassets/Contents.json"));
		supportingFiles.add(new SupportingFile("ios/KindeSDKRN/AppDelegate.h",
				createPath(iOSFolder), "KindeSDKRN/AppDelegate.h"));
		supportingFiles.add(new SupportingFile("ios/KindeSDKRN/AppDelegate.m",
				createPath(iOSFolder), "KindeSDKRN/AppDelegate.m"));
		supportingFiles.add(new SupportingFile("ios/KindeSDKRN/Info.plist", createPath(iOSFolder),
				"KindeSDKRN/Info.plist"));
		supportingFiles.add(new SupportingFile("ios/KindeSDKRN/main.m", createPath(iOSFolder),
				"KindeSDKRN/main.m"));
		...
		supportingFiles.add(new SupportingFile(
				...
				createPath(iOSFolder),
				"KindeSDKRN.xcodeproj/xcshareddata/xcschemes/KindeSDKRN.xcscheme"));
		supportingFiles.add(new SupportingFile("ios/KindeSDKRN.xcodeproj/project.pbxproj",
				createPath(iOSFolder), "KindeSDKRN.xcodeproj/project.pbxproj"));

		// Tests folder
		supportingFiles.add(new SupportingFile("ios/KindeSDKRNTests/Info.plist",
				createPath(iOSFolder), "KindeSDKRNTests/Info.plist"));
		supportingFiles.add(new SupportingFile("ios/KindeSDKRNTests/KindeSDKRNTests.m",
				createPath(iOSFolder), "KindeSDKRNTests/KindeSDKRNTests.m"));
	}

	private void addSupportingFilesForExtending() {
		supportingFiles
				.add(new SupportingFile("husky/pre-commit", createPath(".husky"), "pre-commit"));

		supportingFiles.add(new SupportingFile(".eslintignore", createPath("/"), ".eslintignore"));

		supportingFiles.add(new SupportingFile(".eslintrc.js", createPath("/"), ".eslintrc.js"));

		supportingFiles.add(new SupportingFile(".eslintignore", createPath("/"), ".eslintignore"));

		supportingFiles
				.add(new SupportingFile(".prettierignore", createPath("/"), ".prettierignore"));

		supportingFiles
				.add(new SupportingFile(".prettierrc.json", createPath("/"), ".prettierrc.json"));

		supportingFiles
				.add(new SupportingFile("tsconfig.mustache", createPath("/"), "tsconfig.json"));
	}
